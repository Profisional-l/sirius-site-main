...
export function ServicesAccordion() {
  ...
  const CARD_REVEAL_DURATION_PX = isMobile ? 520 : 320;
  const SECOND_CARD_REVEAL_DURATION_PX = isMobile ? 360 : 220;
  const SECOND_CARD_PEEK_PX = 180;
  const THIRD_CARD_PEEK_PX = 90;
  const CARD_BG_TAIL_HEIGHT_VH = isMobile ? 320 : 8;
  const STAGE_EXIT_BUFFER_PX = isMobile ? 360 : 80;
  const SECOND_CARD_START_EARLY_PX = isMobile ? 0 : 900;
  const THIRD_CARD_START_EARLY_PX = isMobile ? 0 : 700;
  const STICKY_EXIT_EXTRA_RATIO = isMobile ? 1 : 0;
  const MIN_CARD_SCROLL_PX = isMobile ? 120 : 0;
  const DESKTOP_MAX_REVEAL_OFFSET_PX = 220;
  const DESKTOP_VISIBLE_CONTENT_PX = 600;
  const FIRST_CARD_VISIBLE_WITH_PEEKS_PX =
    DESKTOP_VISIBLE_CONTENT_PX + SECOND_CARD_PEEK_PX;
  const SECOND_CARD_VISIBLE_WITH_PEEK_PX =
    DESKTOP_VISIBLE_CONTENT_PX + THIRD_CARD_PEEK_PX;
  ...
  useEffect(() => {
    const updateLayout = () => {
      ...
      const firstCardScrollPx = isMobile
        ? Math.max(
            MIN_CARD_SCROLL_PX,
        ...
        : Math.max(
            0,
            (measuredContentHeights[0] ?? 0) - FIRST_CARD_VISIBLE_WITH_PEEKS_PX,
          );
      const secondCardScrollPx = isMobile
        ? Math.max(
            MIN_CARD_SCROLL_PX,
        ...
        : Math.max(
            0,
            (measuredContentHeights[1] ?? 0) - SECOND_CARD_VISIBLE_WITH_PEEK_PX,
          );
      const thirdCardScrollPx = isMobile
        ? Math.max(0, (measuredContentHeights[2] ?? 0) - nextViewportHeight)
        : Math.max(
            0,
            (measuredContentHeights[2] ?? 0) - DESKTOP_VISIBLE_CONTENT_PX,
          );
      ...
      const computedSectionHeightPx =
        thirdCardContentEndPx + stickyExitExtraPx + STAGE_EXIT_BUFFER_PX;

      setSectionHeightPx(computedSectionHeightPx);
    };
    ...
    FIRST_CARD_VISIBLE_WITH_PEEKS_PX,
    SECOND_CARD_VISIBLE_WITH_PEEK_PX,
    DESKTOP_VISIBLE_CONTENT_PX,
  ]);
  ...
  const revealOffsetPx = isMobile
    ? initialOffsetPx
    : Math.min(initialOffsetPx, DESKTOP_MAX_REVEAL_OFFSET_PX);
  const firstCardScrollPx = isMobile
    ? Math.max(
        MIN_CARD_SCROLL_PX,
        (contentHeightsPx[0] ?? 0) -
          viewportHeightPx +
          SECOND_CARD_PEEK_PX +
          THIRD_CARD_PEEK_PX,
      )
    : Math.max(
        0,
        (contentHeightsPx[0] ?? 0) - FIRST_CARD_VISIBLE_WITH_PEEKS_PX,
      );
  const secondCardScrollPx = isMobile
    ? Math.max(
        MIN_CARD_SCROLL_PX,
    ...
    : Math.max(
        0,
        (contentHeightsPx[1] ?? 0) - SECOND_CARD_VISIBLE_WITH_PEEK_PX,
      );
  const thirdCardScrollPx = isMobile
    ? viewportHeightPx > 0
      ? Math.max(0, (contentHeightsPx[2] ?? 0) - viewportHeightPx)
      : 0
    : Math.max(0, (contentHeightsPx[2] ?? 0) - DESKTOP_VISIBLE_CONTENT_PX);
  const secondInitialOffsetPx = Math.max(
    0,
    revealOffsetPx - SECOND_CARD_PEEK_PX,
  );
  const thirdInitialOffsetPx = Math.max(0, revealOffsetPx - THIRD_CARD_PEEK_PX);
  ...
  const firstCardY = useTransform(
    scrollY,
    [sectionStartY, sectionStartY + firstCardContentEndPx],
    [0, -firstCardScrollPx],
    { clamp: true },
  );
        ...
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div>
            {services.map((service, index) => (
              <motion.div
                key={service.id}
                className={`${service.bgColor} relative w-full rounded-[17px]  shadow-lg ${index > 0 ? "-mt-5" : ""}`}
    ...
    >
      ...
      <div className="sticky top-0 h-screen overflow-visible">
        <motion.div
          ...
          className={`${services[0].bgColor} absolute left-0 right-0 top-0 mx-auto w-full max-w-[1280px] ${isMobile ? "min-h-screen" : ""} flex flex-col overflow-visible rounded-t-[17px] rounded-b-none transform-gpu will-change-transform ${isMobile ? "shadow-none" : "shadow-lg"}`}
        >
          <div
            aria-hidden="true"
        ...
        <motion.div
          ...
          className={`${services[1].bgColor} absolute left-0 right-0 top-0 mx-auto w-full max-w-[1280px] ${isMobile ? "min-h-screen" : ""} flex flex-col overflow-visible rounded-t-[17px] rounded-b-none transform-gpu will-change-transform ${isMobile ? "shadow-none" : "shadow-lg"}`}
        >
          <div
            aria-hidden="true"
